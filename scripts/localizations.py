...
FILENAME = 'all.zip'
DEST_DIR = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../src/extension/features/general/localization/locales')
    ...
    for root, dirs, files in os.walk(DEST_DIR):
        for name in files:
            if lang_completed[name.split('.')[0]] != 0:
                ...
                with open(os.path.join(DEST_DIR, name.lower()), 'r+') as f:
                    content = f.read()
                    f.seek(0, 0)
                    f.write(content)
    for root, dirs, files in os.walk(DEST_DIR):
